...
# ADD: Create a unit start time for the logger, and create a logger
START_TIME = datetime.now().strftime("%m%d%H%M")
LOGGER_PATH = f"logs/{START_TIME}"
train_logger = configure(folder=LOGGER_PATH, format_strings=["stdout", "log", "csv", "tensorboard"])
...
def train(model: SAC, total_timesteps: int, save_dir: Path) -> None:
    ...
    fname = f"sac_humanoid_{START_TIME}"
