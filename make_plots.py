...
def plot_csv_files(directory_path):
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith('.csv'):
            ...
            plt.plot(
                data['Step'], data['Value'], label=f'{filename[4:-4]}'
            )  # 使用文件名作為圖例標籤

            # 圖表標題和軸標籤
            y_label = ' '.join(
                filename[filename.index('tag-') + 4 : -4].split('_')
            )
            plt.xlabel('Step')
            plt.ylabel(y_label)
            plt.legend()
...
def go_throght_dir(base_dir):
    for algo in ALGOS:
        for wrapped in WRAPPED:
            plot_csv_files(
                os.path.join(
                    base_dir, 'FINAL_DATA', 'DATA_CSV', f'{algo}_{wrapped}'
                )
            )
...
def plot_comparison(base_dir, tags):
    ...
    for wrapped in WRAPPED:
        ...
        for algo in ALGOS:
            ...
            for filename in os.listdir(directory_path):
                if filename.endswith('.csv') and any(
                    tag in filename for tag in tags
                ):
                    file_path = os.path.join(directory_path, filename)
                    data = pd.read_csv(file_path)
        ...
        plt.grid(True)
        plt.savefig(
            os.path.join(base_dir, 'src', f'comparison_{tags[0]}_{wrapped}.png')
        )
        plt.close()
...
def plot_sac_compare(base_dir):
    ...
    for tag in tags:
        ...
        for wrapped in WRAPPED:
            directory_path = os.path.join(
                base_dir, 'FINAL_DATA', 'DATA_CSV', f'SAC_{wrapped}'
            )
            for filename in os.listdir(directory_path):
                if filename.endswith('.csv') and tag in filename:
                    file_path = os.path.join(directory_path, filename)
...
def plot_ppo_compare(base_dir):
    ...
    for tag in tags:
        ...
        for wrapped in WRAPPED:
            directory_path = os.path.join(
                base_dir, 'FINAL_DATA', 'DATA_CSV', f'PPO_{wrapped}'
            )
            for filename in os.listdir(directory_path):
                if filename.endswith('.csv') and tag in filename:
                    file_path = os.path.join(directory_path, filename)
